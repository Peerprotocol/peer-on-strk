...
export default function Modal({ title, children, show, onClose }: ModalProps) {
  ...
  return (
    <div className={`fixed top-0 left-0 w-full h-screen overflow-hidden bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center ${show ? "scale-1" : "scale-0"}`}>
      <section ref={ref} className="w-[96%] max-w-2xl flex flex-col gap-4 bg-white dark:bg-neutral-900 rounded-xl px-4 py-4">
        <X onClick={onClose} className="ml-auto w-max cursor-pointer text-black dark:text-white" />

        <h3 className="mx-auto text-2xl text-center whitespace-nowrap font-medium">{title}</h3>
